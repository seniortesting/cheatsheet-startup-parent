...
import { mixinApp, mixinDevice } from '@/mixins/app'
...
export default {
  ...
  methods: {
    ...
    onColorWeak (checked) {
      ...
      this.$store.dispatch(names.TOGGLE_WEAK, checked)
      updateColorWeak(checked)
    },
    onMultiTab (checked) {
      this.baseConfig.multiTab = checked
      this.$store.dispatch(names.TOGGLE_MULTI_TAB, checked)
    },
    handleMenuTheme (theme) {
      this.baseConfig.navTheme = theme
      this.$store.dispatch(names.TOGGLE_THEME, theme)
    },
    doCopy () {
    ...
    handleLayout (mode) {
      ...
      this.$store.dispatch(names.TOGGLE_LAYOUT_MODE, mode)
    ...
    handleContentWidthChange (type) {
      ...
      this.$store.dispatch(names.TOGGLE_CONTENT_WIDTH, type)
    },
    changeColor (color) {
      this.baseConfig.primaryColor = color
      if (this.primaryColor !== color) {
        this.$store.dispatch(names.TOGGLE_COLOR, color)
        updateTheme(color)
      }
    },
    handleFixedHeader (fixed) {
      this.baseConfig.fixedHeader = fixed
      this.$store.dispatch(names.TOGGLE_FIXED_HEADER, fixed)
    },
    handleFixedHeaderHidden (autoHidden) {
      this.baseConfig.autoHideHeader = autoHidden
      this.$store.dispatch(names.TOGGLE_FIXED_HEADER_HIDDEN, autoHidden)
    },
    handleFixSiderbar (fixed) {
      if (this.layoutMode === 'topmenu') {
        this.baseConfig.fixSiderbar = false
        this.$store.dispatch(names.TOGGLE_FIXED_SIDERBAR, false)
        return
      }
      this.baseConfig.fixSiderbar = fixed
      this.$store.dispatch(names.TOGGLE_FIXED_SIDERBAR, fixed)
    }
  }
}
</script>
